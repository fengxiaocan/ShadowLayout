			...
			@NonNull Context context,@Nullable AttributeSet attrs,int defStyleAttr)
	{
		...
		this.mCardViewDelegate = new ShadowViewDelegate() {
			...
			public boolean getUseCompatPadding() {
				return ShadowLinearLayout.this.getUseCompatPadding();
			}
			
			public boolean getPreventCornerOverlap() {
				return ShadowLinearLayout.this.getPreventCornerOverlap();
			}
			
			public void setShadowPadding(int left,int top,int right,int bottom) {
				ShadowLinearLayout.this.mShadowBounds.set(left,top,right,bottom);
				ShadowLinearLayout.super
						.setPadding(left + ShadowLinearLayout.this.mContentPadding.left,
						            top + ShadowLinearLayout.this.mContentPadding.top,
						            right + ShadowLinearLayout.this.mContentPadding.right,
						            bottom + ShadowLinearLayout.this.mContentPadding.bottom);
			}
			
			public void setMinWidthHeightInternal(int width,int height) {
				if (width > ShadowLinearLayout.this.mUserSetMinWidth) {
					ShadowLinearLayout.super.setMinimumWidth(width);
				}
				
				if (height > ShadowLinearLayout.this.mUserSetMinHeight) {
					ShadowLinearLayout.super.setMinimumHeight(height);
				}
			...
			public void setCardBackground(Drawable drawable) {
				...
				ShadowLinearLayout.this.setBackgroundDrawable(drawable);
			}
			
			public View getCardView() {
				return ShadowLinearLayout.this;
			}
		};
		TypedArray a = context
				.obtainStyledAttributes(attrs,R.styleable.ShadowLinearLayout,defStyleAttr,
				                        R.style.ShadowLayoutStyle);
		ColorStateList backgroundColor;
		if (a.hasValue(R.styleable.ShadowLinearLayout_shadowBackgroundColor)) {
			backgroundColor = a
					.getColorStateList(R.styleable.ShadowLinearLayout_shadowBackgroundColor);
		}
		else {
		...
		float radius = a.getDimension(R.styleable.ShadowLinearLayout_shadowCornerRadius,0.0F);
		float elevation = a.getDimension(R.styleable.ShadowLinearLayout_shadowElevation,0.0F);
		float maxElevation = a.getDimension(R.styleable.ShadowLinearLayout_shadowMaxElevation,0.0F);
		
		int insetShadow = a.getDimensionPixelSize(R.styleable.ShadowLinearLayout_insetShadow,3);
		int shadowStartColor = a.getColor(R.styleable.ShadowLinearLayout_shadowStartColor,
		                                  getResources().getColor(R.color.shadow_start_color));
		int shadowEndColor = a.getColor(R.styleable.ShadowLinearLayout_shadowEndColor,
		                                getResources().getColor(R.color.shadow_end_color));
		
		this.mCompatPadding = a
				.getBoolean(R.styleable.ShadowLinearLayout_shadowUseCompatPadding,false);
