...
class RoundRectDrawableWithShadow extends Drawable {
	...
	RoundRectDrawableWithShadow(ColorStateList backgroundColor,float radius,float shadowSize,
			float maxShadowSize,@ColorInt int shadowStartColor,@ColorInt int shadowEndColor,int insetShadow)
	{
		mShadowStartColor = shadowStartColor;
	
		mShadowEndColor = shadowEndColor;
		this.mInsetShadow = insetShadow;
	...
	{
		return addPaddingForCorners ? (float)((double)(maxShadowSize * 1.5F)
		                                      + (1.0D - COS_45) * (double)cornerRadius)
				: maxShadowSize * 1.5F;
	}
	...
	static float calculateHorizontalPadding(
			float maxShadowSize,float cornerRadius,boolean addPaddingForCorners)
	{
		return addPaddingForCorners ? (float)((double)maxShadowSize
		                                      + (1.0D - COS_45) * (double)cornerRadius)
				: maxShadowSize;
	}
	...
	private void setBackground(ColorStateList color) {
		...
		this.mPaint.setColor(this.mBackground.getColorForState(this.getState(),
		                                                       this.mBackground.getDefaultColor()));
	}
	...
	private void setShadowSize(float shadowSize,float maxShadowSize) {
		if (shadowSize < 0.0F) {
			...
			if (this.mRawShadowSize != shadowSize || this.mRawMaxShadowSize != maxShadowSize) {
				...
				this.mShadowSize = (float)((int)(shadowSize * 1.5F
				                                 + (float)this.mInsetShadow
				                                 + 0.5F));
				this.mDirty = true;
				this.invalidateSelf();
			}
	...
	public boolean getPadding(Rect padding) {
		int vOffset = (int)Math
				.ceil((double)calculateVerticalPadding(this.mRawMaxShadowSize,this.mCornerRadius,
				                                       this.mAddPaddingForCorners));
		int hOffset = (int)Math
				.ceil((double)calculateHorizontalPadding(this.mRawMaxShadowSize,this.mCornerRadius,
				                                         this.mAddPaddingForCorners));
	...
	protected boolean onStateChange(int[] stateSet) {
		int newColor = this.mBackground
				.getColorForState(stateSet,this.mBackground.getDefaultColor());
	...
	private void buildShadowCorners() {
		...
		this.mCornerShadowPaint.setShader(
				new RadialGradient(0.0F,0.0F,this.mCornerRadius + this.mShadowSize,new int[]{
						this.mShadowStartColor,this.mShadowStartColor,this.mShadowEndColor
				},new float[]{0.0F,startRatio,1.0F},Shader.TileMode.CLAMP));
		this.mEdgeShadowPaint.setShader(
				new LinearGradient(0.0F,-this.mCornerRadius + this.mShadowSize,0.0F,
				                   -this.mCornerRadius - this.mShadowSize,new int[]{
						this.mShadowStartColor,this.mShadowStartColor,this.mShadowEndColor
	...
	private void buildComponents(Rect bounds) {
		...
		this.mCardBounds
				.set((float)bounds.left + this.mRawMaxShadowSize,(float)bounds.top + verticalOffset,
				     (float)bounds.right - this.mRawMaxShadowSize,
				     (float)bounds.bottom - verticalOffset);
	...
	float getMinWidth() {
		float content = 2.0F * Math.max(this.mRawMaxShadowSize,this.mCornerRadius
		                                                       + (float)this.mInsetShadow
		                                                       + this.mRawMaxShadowSize / 2.0F);
